//! Regression test for a Windows-only debug-build bug where `wezterm cli
//! <subcommand>` overflowed the process main thread's stack.
//!
//! Root cause: `wezterm::cli::run_cli` drove a single large async state
...
fn cli_help_does_not_crash() {
    let output = Command::new(wezterm_exe())
        .args(["cli", "--help"])
        .output()
        .expect("failed to run wezterm cli --help");

    assert!(
        output.status.success(),
        "wezterm cli --help did not exit successfully: {:?}\nstdout: {}\nstderr: {}",
        output.status,
...
fn cli_list_does_not_crash_without_server() {
    ...
    let output = Command::new(wezterm_exe())
        ...
        .output()
        .expect("failed to run wezterm cli --no-auto-start list");

    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        !stderr.contains("overflowed its stack"),
        "wezterm cli list overflowed its stack: {}",
        stderr
    );
    ...
    #[cfg(windows)]
    {
        ...
        assert_ne!(
            ...
            "wezterm cli list crashed with STATUS_STACK_OVERFLOW: {:?}",
            output.status
        );
    }
}
