...
//! `onlyterm-font`: it exercises the production
//! `onlyterm_font::rasterizer::swash::SwashRasterizer` (selected via
//! `onlyterm_font::rasterizer::new_rasterizer` /
//! `onlyterm_config::FontRasterizerSelection::Swash`, the crate's default), which
//! handles both ordinary glyph outlines and COLR/COLRv1/CBDT/sbix color
//! glyphs (the latter via its internal, pure-Rust
//! `onlyterm_font::rasterizer::colr_paint::ColrRasterizer` fallback) in a
...
//! PNGs/JSON live under `onlyterm-font/testdata/baseline/` (some filenames
...
use onlyterm_font::locator::{FontDataHandle, FontDataSource, FontOrigin};
use onlyterm_font::parser::ParsedFont;
use onlyterm_font::rasterizer::{new_rasterizer, RasterizedGlyph};
use onlyterm_font::swash_metrics::SwashFontInfo;
use std::path::{Path, PathBuf};

#[derive(Copy, Clone, Debug, PartialEq, Eq, ValueEnum)]
enum RasterizerArg {
    /// The production, pure-Rust `swash`-based rasterizer (see
    /// `onlyterm_font::rasterizer::swash::SwashRasterizer`). This is the
    /// only rasterizer left in the workspace: it is selected via
    /// `onlyterm_font::rasterizer::new_rasterizer` /
    /// `onlyterm_config::FontRasterizerSelection::Swash`, the crate's default.
    Swash,
}

impl RasterizerArg {
    fn selection(self) -> onlyterm_config::FontRasterizerSelection {
        match self {
            RasterizerArg::Swash => onlyterm_config::FontRasterizerSelection::Swash,
        }
    }
}

fn selection_name(sel: onlyterm_config::FontRasterizerSelection) -> &'static str {
    match sel {
        onlyterm_config::FontRasterizerSelection::Swash => "swash",
        onlyterm_config::FontRasterizerSelection::FreeType => "freetype",
        onlyterm_config::FontRasterizerSelection::Harfbuzz => "harfbuzz",
    }
}
...
struct Opt {
    ...
    /// `onlyterm_font::rasterizer::swash::SwashRasterizer`.
...
fn dump_one(
    ...
    glyph_id: u32,
    opt: &Opt,
    selection: onlyterm_config::FontRasterizerSelection,
    out_png: &PathBuf,
    out_json: &PathBuf,
) -> anyhow::Result<()> {
