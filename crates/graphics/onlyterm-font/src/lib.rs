...
use onlyterm_term::{CellAttributes, Intensity};
...
impl FontConfigInner {
            ...
            if !attr.is_synthetic && !attr.is_fallback && !loaded.contains(attr) {
                ...
                let explanation = if is_primary {
                    ...
                    format!(
                        ...
                        Note that onlyterm will synthesize font_rules to select bold \
                ...
                onlyterm_config::show_error(&format!(
    ...
    pub fn match_style<'a>(
        ...
        let would_bright = match attrs.foreground() {
            onlyterm_term::color::ColorAttribute::PaletteIndex(idx) if idx < 8 => {
