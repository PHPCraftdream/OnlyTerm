...
mod stack_overflow_repro {
    ...
    //! alongside this one, `crates/configuration/config/src/config.rs`'s
    ...
    //! stack. The actual trigger was `onlyterm_config::configuration()`'s *first*
    ...
    //! `FromDynamic` derive macro (`crates/configuration/onlyterm-dynamic/derive`),
