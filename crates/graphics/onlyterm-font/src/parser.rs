...
/// itself to the small set of name ids onlyterm cares about (family/
...
impl ParsedFont {
    ...
    pub fn from_face(font_info: &SwashFontInfo, handle: FontDataHandle) -> anyhow::Result<Self> {
        ...
        if has_svg && onlyterm_config::configuration().ignore_svg_fonts {
    ...
    /// font provides one. Diagnostic-only (used by `onlyterm-gui`'s `ls-fonts`/
...
mod scan;
pub(crate) use scan::parse_and_collect_font_info;
