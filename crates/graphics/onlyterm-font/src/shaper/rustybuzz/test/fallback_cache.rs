...
fn fallback_only_runs_keep_parsed_primary_font() {
    ...
    let shaper = RustybuzzShaper::new(&onlyterm_config::configuration(), &handles).unwrap();
...
fn appending_fallback_keeps_existing_face_and_plan_cache() {
    let handles = primary_then_hebrew_fallback_handles();
    let mut shaper = RustybuzzShaper::new(&onlyterm_config::configuration(), &handles).unwrap();
    let mut missing = Vec::new();
    shaper
        .shape(
            "plain text",
