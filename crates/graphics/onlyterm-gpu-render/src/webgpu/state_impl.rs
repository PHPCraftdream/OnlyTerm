...
/// `onlyterm_promise::Future` that resolves with its result once the thread finishes.
///
...
/// -- see task #311) off the GUI thread. `onlyterm_promise::spawn`'s executor (backed
...
/// duration of `f`, and `onlyterm_promise::Promise::result`'s `waker.wake()` call
...
/// first).
pub(super) fn run_on_background_thread<T, F>(f: F) -> onlyterm_promise::Future<T>
where
    T: Send + 'static,
    F: FnOnce() -> T + Send + 'static,
{
    let mut promise = onlyterm_promise::Promise::new();
