use super::config_paths::LegacyScriptSiblingError;
pub(super) use super::config_paths::PathPossibility;
use super::Config;
...
impl Config {
    ...
    pub(super) fn search_paths_for_config(
        ...
        // `crates/configuration/config/src/lib.rs`) builds its filesystem-watch list from
    ...
    /// `emit_async_callback`, see `crates/session/mux/src/domain.rs`) that rewrote
    ...
    /// `LocalDomain::fixup_command` in `crates/session/mux/src/domain.rs`), so
