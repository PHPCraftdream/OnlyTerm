...
/// Resolve a map key (`BTreeMap`/`HashMap`) from a `Value`, with a fallback
/// for integer-keyed maps: if `K::from_dynamic` rejects the key outright and
/// the key is a `Value::String` that parses cleanly as either an `i64` or a
/// `u64`, retry the conversion against a synthesized `Value::I64`/`Value::U64`
/// built from that parsed number (which `K`'s own `FromDynamic` impl -- e.g.
/// the integer impls generated by the `int!` macro below -- already knows
/// how to accept).
///
/// This matters because config-format front ends that only ever produce
/// string object keys (e.g. `ktav`, whose object keys are unconditionally
/// `Value::String`; see `crates/configuration/config/src/ktav_value.rs`) cannot otherwise
/// populate an integer-keyed map like `colors.indexed: HashMap<u8,
/// RgbaColor>` (see `crates/configuration/config/src/color.rs`), even though the intent
/// ("a numeric-looking key like `136`") is unambiguous. The old rhai config
/// path produced real integer keys for numeric-looking object keys, so this
/// restores that behavior without loosening integer parsing anywhere else:
/// struct fields, map *values*, and non-map uses of integer types are
/// unaffected, and a string that isn't a valid integer still falls through
/// to `K::from_dynamic`'s existing (strict) error.
fn map_key_from_dynamic<K: FromDynamic>(
    value: &Value,
    options: FromDynamicOptions,
) -> Result<K, Error> {
    match K::from_dynamic(value, options) {
        Ok(k) => Ok(k),
        Err(err) => match value {
            Value::String(s) => {
                if let Ok(n) = s.parse::<i64>() {
                    if let Ok(k) = K::from_dynamic(&Value::I64(n), options) {
                        return Ok(k);
                    }
                }
                if let Ok(n) = s.parse::<u64>() {
                    if let Ok(k) = K::from_dynamic(&Value::U64(n), options) {
                        return Ok(k);
                    }
                }
                Err(err)
    ...
    fn from_dynamic(value: &Value, options: FromDynamicOptions) -> Result<Self, Error> {
        match value {
            Value::Object(obj) => {
                ...
                for (k, v) in obj.iter() {
                    map.insert(
                        map_key_from_dynamic::<K>(k, options)?,
                        T::from_dynamic(v, options)?,
                    );
                }
                Ok(map)
            }
    ...
    fn from_dynamic(value: &Value, options: FromDynamicOptions) -> Result<Self, Error> {
        match value {
            Value::Array(arr) => Ok(arr
                ...
                .map(|v| T::from_dynamic(v, options))
                .collect::<Result<Vec<T>, Error>>()?),
            // An empty object can stand in for "no data" from callers that don't
            // distinguish "empty array" from "empty object" when synthesizing a
            // placeholder `Value` (e.g. `Value::Object(Default::default())` used
            // as a blank starting document, see `Config::default()`). Accept it
            // here as an empty array rather than erroring.
