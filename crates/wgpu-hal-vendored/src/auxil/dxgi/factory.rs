...
// Was private; widened to `pub(crate)` so `dx12::instance`'s
// `fast_preferred_adapter` (a OnlyTerm-only addition, see that method's
// doc comment) can apply the same Intel-Haswell/headless-WARP filtering
// to the single adapter it fetches via `EnumAdapterByGpuPreference`,
// rather than duplicating this logic.
pub(crate) fn should_keep_adapter(adapter: &Dxgi::IDXGIAdapter1) -> bool {
...
impl DxgiFactory {
    ...
    // Added for `dx12::instance::fast_preferred_adapter`: `IDXGIFactory6`
    // is what exposes `EnumAdapterByGpuPreference`.
    pub fn as_factory6(&self) -> Option<&Dxgi::IDXGIFactory6> {
        match self {
            Self::Factory4(_) | Self::Factory5(_) => None,
            Self::Factory6(f) => Some(f),
        }
    }
}
