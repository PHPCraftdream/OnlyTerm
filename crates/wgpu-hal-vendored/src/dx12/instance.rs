...
impl super::Instance {
    /// OnlyTerm addition (not upstream wgpu-hal): fetch and `expose()` a
    /// single adapter chosen by `IDXGIFactory6::EnumAdapterByGpuPreference`,
    /// instead of `enumerate_adapters`'s `EnumAdapters1` scan that probes
    /// (creates a real `ID3D12Device` for) every adapter DXGI reports.
    ///
    /// `EnumAdapterByGpuPreference` resolves its ordering from adapter
    /// metadata DXGI/the OS already has -- it does not itself create a
    /// device on any adapter, so asking it for the single best adapter for
    /// `preference` and calling `expose()` on only that one, rather than
    /// on every adapter `enumerate_adapters` would return, avoids waking a
    /// suspended discrete GPU purely to measure its capabilities on
    /// hybrid-graphics laptops where the default `LowPower` preference was
    /// never going to pick it anyway.
    ///
    /// Returns `None` (never a partial/wrong result) if `IDXGIFactory6`
    /// isn't available (pre-1803 Windows 10), the preferred adapter is
    /// filtered out by the same rules `enumerate_adapters` applies (see
    /// `auxil::dxgi::factory::should_keep_adapter`), or it fails to
    /// `expose()` for any reason -- callers must fall back to
    /// `enumerate_adapters` (the full, correct scan) in that case.
    pub fn fast_preferred_adapter(
        &self,
        preference: Dxgi::DXGI_GPU_PREFERENCE,
    ) -> Option<crate::ExposedAdapter<super::Api>> {
        let factory6 = self.factory.as_factory6()?;

        let adapter1: Dxgi::IDXGIAdapter1 =
            unsafe { factory6.EnumAdapterByGpuPreference(0, preference) }.ok()?;

        if !auxil::dxgi::factory::should_keep_adapter(&adapter1) {
            return None;
        }

        let raw = if let Ok(adapter4) = adapter1.cast::<Dxgi::IDXGIAdapter4>() {
            auxil::dxgi::factory::DxgiAdapter::Adapter4(adapter4)
        } else {
            let adapter3 = adapter1.cast::<Dxgi::IDXGIAdapter3>().ok()?;
            auxil::dxgi::factory::DxgiAdapter::Adapter3(adapter3)
        };
