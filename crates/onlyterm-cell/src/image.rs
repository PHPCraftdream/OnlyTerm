...
#[cfg(feature = "std")]
use onlyterm_blob_leases::{BlobLease, BlobManager};
...
impl ImageCell {
    ...
    // onlyterm-client) with these exact positional arguments. Regrouping the
...
pub enum ImageDataType {
    ...
    EncodedLease(
        #[cfg_attr(
            feature = "use_serde",
            serde(with = "onlyterm_blob_leases::lease_bytes")
        )]
        BlobLease,
    ),
    ...
    pub fn swap_out(self) -> Result<Self, ImageCellError> {
        match self {
            Self::EncodedFile(data) => match BlobManager::store(&data) {
                ...
                Err(onlyterm_blob_leases::Error::StorageNotInit) => Ok(Self::EncodedFile(data)),
...
pub enum ImageCellError {
    ...
    #[error(transparent)]
    BlobLease(#[from] onlyterm_blob_leases::Error),
