...
pub(crate) fn parse_and_collect_font_info(
    ...
    //
    // We keep the loaded bytes around (rather than immediately dropping
    // them, as a previous version of this function did) and reuse them for
    // every sub-face below via `SwashFontInfo::from_data`, instead of
    // letting each `load_one` call independently re-read the same file off
    // disk through `SwashFontInfo::from_locator`. For a `.ttc`/`.otc` font
    // collection with N sub-faces, the old pattern read the whole file
    // (which, for CJK system collections such as `mingliub.ttc` or
    // `Sitka*.ttc`, can be tens of megabytes) N+1 times over -- once here
    // just to learn the face count, and once again per sub-face -- even
    // though every sub-face lives in the very same file. This was a
    // measurable contributor to `enumerate_all_fonts`/`ls-fonts
    // --list-system` being slower than the old FreeType-based
    // implementation (see the PERF1 investigation notes); re-reading from
    // disk was pure waste since the bytes are identical across sub-faces.
    ...
    fn load_one(
        ...
        // Clone the already-in-memory bytes (a plain memcpy, no I/O) rather
        // than going through `SwashFontInfo::from_locator`, which would
        // re-read the file from disk for every sub-face.
        let font_ref_info = SwashFontInfo::from_data(data.to_vec().into_boxed_slice(), index)?;
