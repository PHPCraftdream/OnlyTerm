...
//
// Run individual tests with: cargo test -p onlyterm-elevated-transport transport_performance -- --test-threads=1 --nocapture

use anyhow::Context as _;
use filedescriptor::socketpair;
use onlyterm_codec::{Pdu, Ping, Pong, WriteToPane};
...
fn decode_pdu_timeout<R: Read>(
    ...
    timeout: Duration,
) -> anyhow::Result<onlyterm_codec::DecodedPdu> {
    let deadline = Instant::now() + timeout;
...
fn bench_throughput_websocket_bridge() {
    ...
    let listener = onlyterm_elevated_transport::RendezvousListener::bind()
    ...
    let mut client_stream = onlyterm_elevated_transport::connect_and_bridge(port, &token)
