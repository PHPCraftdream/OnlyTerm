...
use onlyterm_config::{ConfigHandle, TabBarColors};
use onlyterm_term::{Line, Progress};
use onlyterm_termwiz_funcs::{format_as_escapes, FormatColor, FormatItem};
...
fn compute_tab_title(tab: &TabInformation, config: &ConfigHandle) -> TitleText {
    ...
    if let Some(pane) = &tab.active_pane {
        let mut title = if !tab.tab_title.is_empty() {
            // An explicit rename (`onlyterm cli set-tab-title`, the rename
