...
use onlyterm_client::domain::{ClientDomain, ClientDomainConfig};
use onlyterm_config::keyassignment::SpawnCommand;
use onlyterm_config::{ProcessPriority, TermConfig, UnixDomain};
use onlyterm_mux::activity::Activity;
use onlyterm_mux::domain::{alloc_domain_id, Domain, SplitSource};
use onlyterm_mux::tab::{SplitRequest, Tab};
...
/// startup (see `run_single_pane_mode` in onlyterm-mux-server) -- attaching
...
fn resolve_spawn_title(spawn: &SpawnCommand) -> Option<String> {
    spawn
        .title
        .clone()
        .or_else(|| onlyterm_config::configuration().default_tab_title.clone())
}
...
mod tests {
    ...
    fn failed_middle_preparation_preserves_earlier_ordering() {
        ...
        drop(first);
        onlyterm_promise::spawn::block_on(third_turn);
    }
}
