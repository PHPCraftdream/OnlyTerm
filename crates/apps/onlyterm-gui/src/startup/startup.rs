...
async fn spawn_tab_in_domain_if_mux_is_empty(
    ...
    let _config_subscription = onlyterm_config::subscribe_to_config_reload(move || {
        onlyterm_promise::spawn::spawn_into_main_thread(async move {
            if let Err(err) = update_mux_domains(&onlyterm_config::configuration()) {
...
async fn spawn_startup_layout(
    layout: &onlyterm_config::StartupLayout,
    domain: Option<Arc<dyn Domain>>,
    workspace: Option<String>,
) -> anyhow::Result<()> {
    ...
    for (idx, tab_conf) in layout.tabs.iter().enumerate() {
        ...
        if !options.admin {
            if config.per_tab_process_isolation {
                ...
                pending_non_elevated.push(onlyterm_promise::spawn::spawn(async move {
        ...
        let tab = {
            ...
            crate::spawn::spawn_elevated_single_pane_tab(
                ...
                cwd.cloned(),
                Some(window_id),
                Arc::new(onlyterm_config::TermConfig::with_config(config.clone())),
            )
            .await
            .with_context(|| {
                format!(
...
async fn spawn_startup_non_elevated_tab(
    ...
    window_id: onlyterm_mux::window::WindowId,
    size: TerminalSize,
    config: onlyterm_config::ConfigHandle,
    ...
    let tab = if config.per_tab_process_isolation {
        let spawn = onlyterm_config::keyassignment::SpawnCommand {
...
pub(super) async fn async_run_terminal_gui(
    ...
    let unix_socket_path =
        onlyterm_config::RUNTIME_DIR.join(format!("gui-sock-{}", std::process::id()));
    std::env::set_var("ONLYTERM_UNIX_SOCKET", unix_socket_path.clone());
    onlyterm_blob_leases::register_storage(Arc::new(
        onlyterm_blob_leases::simple_tempdir::SimpleTempDir::new_in(&*onlyterm_config::CACHE_DIR)?,
    ))?;
    ...
    if let Some(start_conf) = &opts.start_conf {
        let layout = onlyterm_config::StartupLayout::load(start_conf)?;
    ...
    if opts.choose_tab {
        crate::startup_chooser::arm(onlyterm_mux::activity::Activity::new(), opts.cwd.clone());
...
fn spawn_mux_server(unix_socket_path: PathBuf, should_publish: bool) -> anyhow::Result<()> {
    let mut listener = onlyterm_mux_server_impl::local::LocalListener::with_domain(
        &onlyterm_config::UnixDomain {
...
fn setup_mux(
    ...
    let mux = Arc::new(onlyterm_mux::Mux::new(Some(local_domain.clone())));
    Mux::set_mux(&mux);
    let client_id = Arc::new(onlyterm_mux::client::ClientId::new());
    ...
    let default_workspace_name = default_workspace_name.unwrap_or(
        config
            .default_workspace
            .as_deref()
            .unwrap_or(onlyterm_mux::DEFAULT_WORKSPACE),
    );
...
fn select_startup_tab(window: &mut onlyterm_mux::window::Window) {
    if !window.is_empty() {
...
mod tests {
    ...
    fn startup_layout_selects_first_tab_without_reordering() {
        ...
        let _reset = ResetMux;
        for count in [0, 1, 3] {
            let mut window = onlyterm_mux::window::Window::new(Some("startup-test".into()), None);
            for _ in 0..count {
                window.push(&Arc::new(onlyterm_mux::tab::Tab::new(
