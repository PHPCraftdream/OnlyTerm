...
use onlyterm_promise::{Future, Promise};
use onlyterm_term::{Alert, ClipboardSelection};
...
impl GuiFrontEnd {
    pub fn try_new() -> anyhow::Result<Rc<GuiFrontEnd>> {
        ...
        mux.subscribe(move |n| {
            match n {
                ...
                MuxNotification::Empty => {
                    if onlyterm_config::configuration().quit_when_all_windows_are_closed {
                        onlyterm_promise::spawn::spawn_into_main_thread(async move {
                            if onlyterm_mux::activity::Activity::count() == 0 {
                ...
                MuxNotification::SaveToDownloads { name, data } => {
                    if !onlyterm_config::configuration().allow_download_protocols {
                        log::error!(
        ...
        // `onlyterm.gui.get_appearance()` can have that take effect
