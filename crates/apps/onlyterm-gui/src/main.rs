...
use onlyterm_config::{ConfigHandle, SerialDomain};
...
#[command(
    ...
    version = onlyterm_config::onlyterm_version()
)]
struct Opt {
...
fn run_serial(config: onlyterm_config::ConfigHandle, opts: SerialCommand) -> anyhow::Result<()> {
    ...
    onlyterm_promise::spawn::spawn(async {
...
pub(crate) fn terminate_with_error(err: anyhow::Error) -> ! {
    ...
    let warnings = onlyterm_config::configuration_warnings_and_errors();
...
fn main() {
    ...
    // `onlyterm_config::designate_this_as_the_main_thread()` used to be called here
    ...
    onlyterm_config::assign_error_callback(onlyterm_mux::connui::show_configuration_error_message);
    notify_on_panic();
    if let Err(e) = run() {
...
fn maybe_show_configuration_error_window() {
    ...
    if let Some(err) = onlyterm_config::configuration_error() {
        onlyterm_mux::connui::show_configuration_error_message(&err);
    }

    for warning in onlyterm_config::configuration_warnings_and_errors() {
        log::warn!("{warning}");
    }
}
...
pub fn run_ls_fonts(
    ...
    if let Err(err) = onlyterm_config::configuration_result() {
        log::error!("{}", err);
        return Ok(());
    }
    ...
    onlyterm_config::assign_error_callback(|err| eprintln!("{}", err));
...
fn run() -> anyhow::Result<()> {
    ...
    // and the per-PID log file's own timestamps (see `onlyterm_env_bootstrap::ringlog`,
    ...
    stats::Stats::init()?;
    let _saver = onlyterm_umask::UmaskSaver::new();

    onlyterm_config::common_init(
