...
use onlyterm_config::TextStyle;
use onlyterm_font::units::*;
use onlyterm_font::{FontConfiguration, LoadedFontId};
...
mod hashmap_bench {
    ...
    fn make_glyph_key(id: u32) -> GlyphKey {
        GlyphKey {
            ...
            id: onlyterm_font::LoadedFontId::from((id % 2) as usize), // 2 different loaded fonts
        }
    }
    ...
    fn make_cached_glyph(id: u32) -> Rc<CachedGlyph> {
        Rc::new(CachedGlyph {
            ...
            x_offset: onlyterm_font::units::PixelLength::new((((id % 5) as f32) / 10.0) as f64),
            y_offset: onlyterm_font::units::PixelLength::new((((id % 5) as f32) / 10.0) as f64),
            x_advance: onlyterm_font::units::PixelLength::new((8.0 + ((id % 3) as f32)) as f64),
            bearing_x: onlyterm_font::units::PixelLength::new((((id % 5) as f32) / 10.0) as f64),
            bearing_y: onlyterm_font::units::PixelLength::new((12.0 + ((id % 3) as f32)) as f64),
    ...
    fn make_line_key(id: u32) -> LineKey {
        LineKey {
            ...
            underline: match id % 4 {
                0 => onlyterm_term::Underline::None,
                1 => onlyterm_term::Underline::Single,
                2 => onlyterm_term::Underline::Double,
                _ => onlyterm_term::Underline::Curly,
            },
    ...
    #[ignore = "benchmark, not a correctness test -- takes ~157s; run explicitly with \
                `cargo test -p onlyterm-gui --lib -- --ignored bench_glyph_cache_hashmap_vs_ahashmap`"]
