...
/// exposed every `register_rhai` binding (`onlyterm.mux.*`, `GuiWin`,
/// `TabInformation`, the API helper crates grouped with their domains, the
...
/// summary of the running onlyterm (version, target triple, window/renderer
...
fn print_new_log_entries(term: &mut TermWizTerminal) -> termwiz::Result<()> {
    let entries = onlyterm_env_bootstrap::ringlog::get_entries();
    let mut changes = vec![];
...
pub fn show_debug_overlay(
    ...
    let version = onlyterm_config::onlyterm_version();
    let triple = onlyterm_config::onlyterm_target_triple();

    term.render(&[Change::Text(format!(
        "Debug Overlay\r\n\
         onlyterm version: {version} {triple}\r\n\
