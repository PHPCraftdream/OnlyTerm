...
pub fn show_version_overlay(mut term: TermWizTerminal, gui_win: GuiWin) -> anyhow::Result<()> {
    ...
    let commit_hash = onlyterm_config::onlyterm_commit_hash();
    let commit_count = onlyterm_config::onlyterm_commit_count();
    let build_time = onlyterm_config::onlyterm_build_time();
