use clap::Parser;
use onlyterm_client::client::{unix_connect_with_retry, Client};
use onlyterm_client::domain::ClientDomainConfig;
use onlyterm_codec::{Pdu, SetClientId};
...
impl ProxyCommand {
    pub async fn run(&self, client: Client, _config: &ConfigHandle) -> anyhow::Result<()> {
        ...
        let mux = Arc::new(onlyterm_mux::Mux::new(None));
        Mux::set_mux(&mux);
