...
struct CliListClientsResultItem {
    ...
    workspace: String,
    focused_pane_id: Option<onlyterm_mux::pane::PaneId>,
    ssh_auth_sock: Option<String>,
}

impl From<onlyterm_mux::client::ClientInfo> for CliListClientsResultItem {
    fn from(client_info: onlyterm_mux::client::ClientInfo) -> CliListClientsResultItem {
        let now: DateTime<Utc> = Utc::now();

        let onlyterm_mux::client::ClientInfo {
            connected_at,
            last_input,
            active_workspace,
            focused_pane_id,
        ...
        let onlyterm_mux::client::ClientId {
            username,
            hostname,
            pid,
            ssh_auth_sock,
