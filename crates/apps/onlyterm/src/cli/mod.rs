...
async fn run_cli_async(opts: &crate::Opt, cli: CliCommand) -> anyhow::Result<()> {
    ...
    let mut ui = onlyterm_mux::connui::ConnectionUI::new_headless();
    let initial = true;
    ...
    let client = Client::new_default_unix_domain(
        ...
        cli.class
            .as_deref()
            .unwrap_or(onlyterm_gui_subcommands::DEFAULT_WINDOW_CLASS),
    )?;
...
pub fn run_cli(opts: &crate::Opt, cli: CliCommand) -> anyhow::Result<()> {
    ...
    let result = std::thread::scope(|scope| {
        std::thread::Builder::new()
            ...
            .spawn_scoped(scope, || {
                let executor = onlyterm_promise::spawn::ScopedExecutor::new();
                onlyterm_promise::spawn::block_on(
