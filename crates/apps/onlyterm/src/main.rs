...
use onlyterm_config::{onlyterm_version, ConfigHandle};
use onlyterm_gui_subcommands::*;
use onlyterm_mux::Mux;
use onlyterm_umask::UmaskSaver;
...
// which is still `onlyterm` -- the crate directories keep the upstream names
...
// user-facing `onlyterm shell-completion --shell <name>` value, which is a
...
fn init_config(opts: &Opt) -> anyhow::Result<ConfigHandle> {
    ...
    .context("onlyterm_config::common_init")?;
