...
use onlyterm_config::GuiPosition;
use std::ffi::OsString;
use std::path::PathBuf;

pub const DEFAULT_WINDOW_CLASS: &str = "org.onlyterm.onlyterm";
...
pub struct StartCommand {
    ...
    /// in your onlyterm configuration file.
    #[arg(long = "no-auto-connect")]
    pub no_auto_connect: bool,

    /// If enabled, don't try to ask an existing onlyterm GUI instance
    /// to start the command.  Instead, always start the GUI in this
    /// invocation of onlyterm so that you can wait for the command
    /// to complete by waiting for this onlyterm process to finish.
    ...
    /// is run from. See the "onlyterm start" page of the documentation for
    ...
    /// The default is "org.onlyterm.onlyterm".
    ...
    /// instance of onlyterm, including error and update dialogs.
    #[arg(long = "class")]
    ...
    /// onlyterm will simply attach and will NOT spawn the specified PROG.
    /// If you omit --attach when using --domain, onlyterm will attach
    /// AND then spawn PROG.
