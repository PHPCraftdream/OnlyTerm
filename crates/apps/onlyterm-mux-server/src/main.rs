...
struct Opt {
    ...
    /// (matches onlyterm_config::keyassignment::ProcessPriority's variant names).
...
fn run() -> anyhow::Result<()> {
    ...
    for name in &onlyterm_config::configuration().mux_env_remove {
        std::env::remove_var(name);
    }
    ...
    let mux = Arc::new(onlyterm_mux::Mux::new(Some(domain.clone())));
    Mux::set_mux(&mux);

    let executor = onlyterm_promise::spawn::SimpleExecutor::new();
...
async fn async_run(cmd: Option<CommandBuilder>) -> anyhow::Result<()> {
    ...
    let _config_subscription = onlyterm_config::subscribe_to_config_reload(move || {
        onlyterm_promise::spawn::spawn_into_main_thread(async move {
            if let Err(err) = update_mux_domains_for_server(&onlyterm_config::configuration()) {
...
fn run_single_pane_mode(opts: Opt) -> anyhow::Result<()> {
    ...
    let _config = onlyterm_config::configuration();
...
) -> onlyterm_promise::spawn::Task<anyhow::Result<()>> {
    onlyterm_promise::spawn::spawn_into_main_thread(async move {
