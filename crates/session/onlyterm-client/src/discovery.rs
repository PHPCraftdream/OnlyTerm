...
mod windows {
    ...
    impl NameHolder {
        ...
        fn compute_names(class_name: &str) -> (String, String) {
            let mutex_name = format!("Local\\onlyterm-sock-mutex-{}", class_name);
            let map_name = format!("Local\\onlyterm-sock-{}", class_name);
            (mutex_name, map_name)
        }
        ...
        pub fn new(path: &Path, class_name: &str) -> anyhow::Result<Self> {
            ...
            // `onlyterm cli` work from any shell/cwd.
            let path = path
                .to_str()
    ...
    fn resolve_relative_to_runtime(path: PathBuf) -> PathBuf {
        if path.is_absolute() {
            ...
            onlyterm_config::RUNTIME_DIR.join(path)
        }
    }
    ...
    mod tests {
        ...
        fn relative_name_is_anchored_to_runtime_dir() {
            ...
            assert!(
                anchored.starts_with(&*onlyterm_config::RUNTIME_DIR),
...
pub fn discover_gui_socks() -> Vec<PathBuf> {
    ...
    if let Ok(dir) = std::fs::read_dir(&*onlyterm_config::RUNTIME_DIR) {
