...
use onlyterm_codec::*;
use onlyterm_config::{configuration, UnixDomain};
...
impl Client {
        ...
        thread::spawn(move || {
            ...
            loop {
                if let Err(e) = client_thread(&mut reconnectable, local_domain_id, &mut receiver) {
                    ...
                    ui.title("onlyterm: Reconnecting...");
    ...
    fn compute_unix_domain(
        prefer_mux: bool,
        class_name: &str,
    ) -> anyhow::Result<onlyterm_config::UnixDomain> {
        match std::env::var_os("ONLYTERM_UNIX_SOCKET") {
            Some(path) if !path.is_empty() => Ok(onlyterm_config::UnixDomain {
            ...
            Some(_) | None => {
                if !prefer_mux {
                    if let Ok(gui) = crate::discovery::resolve_gui_sock_path(class_name) {
                        return Ok(onlyterm_config::UnixDomain {
