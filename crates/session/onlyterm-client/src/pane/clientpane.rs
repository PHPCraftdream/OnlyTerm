...
use onlyterm_mux::domain::DomainId;
...
use onlyterm_ratelim::RateLimiter;
    ...
    pub async fn process_unilateral(&self, pdu: Pdu) -> anyhow::Result<()> {
        match pdu {
            ...
            Pdu::PaneFocused(PaneFocused { pane_id }) => {
                ...
                // `onlyterm cli activate-pane-direction` or similar call
    ...
    fn kill(&self) {
...

        {
            ...
            if let Some(client_domain) = mux.get_domain(local_domain_id) {
                if client_domain.state() == onlyterm_mux::domain::DomainState::Detached {
                    send_kill = false;
                }
            }
        }
...
impl std::io::Write for PaneWriter {
    fn write(&mut self, data: &[u8]) -> Result<usize, std::io::Error> {
        onlyterm_promise::spawn::block_on(self.client.client.write_to_pane(WriteToPane {
            pane_id: self.remote_pane_id,
            data: data.to_vec(),
        }))
