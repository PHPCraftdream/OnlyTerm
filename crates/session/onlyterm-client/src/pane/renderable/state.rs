...
use onlyterm_promise::BrokenPromise;
use onlyterm_rangeset::*;
use onlyterm_term::{Line, StableRowIndex};
...
impl RenderableState {
    ...
    pub fn get_lines(&self, lines: Range<StableRowIndex>) -> (StableRowIndex, Vec<Line>) {
        ...
        for idx in lines.clone() {
            ...
            {
                let status = format!(
                    "onlyterm: {:.0?}⏳since last response",
                ...
                let col = inner
                    .dimensions
                    .cols
                    .saturating_sub(onlyterm_term::unicode_column_width(&status, None));
