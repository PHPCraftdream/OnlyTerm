...
use onlyterm_rangeset::*;
use onlyterm_ratelim::RateLimiter;
use onlyterm_term::{KeyCode, KeyModifiers, Line, StableRowIndex};
...
impl RenderableInner {
    ...
    pub fn apply_changes_to_surface(
        ...
        Mux::get().notify(onlyterm_mux::MuxNotification::PaneOutput(
            self.local_pane_id,
        ));
    ...
    fn apply_lines(
        ...
        mux.notify(onlyterm_mux::MuxNotification::PaneOutput(local_pane_id));
        Ok(())
    }
...
mod tests {
    ...
    //! requires an `Arc<ClientInner>` (the full onlyterm-client connection
