...
/// AgentProxy manages an agent.PID symlink in the onlyterm runtime
/// directory.
...
///
/// As a further complication, when a onlyterm proxy client is
...
impl AgentProxy {
    pub fn new() -> Self {
        ...
        let sock_path = onlyterm_config::RUNTIME_DIR.join(format!("agent.{pid}"));
    ...
    pub fn default_ssh_auth_sock() -> Option<String> {
        match &onlyterm_config::configuration().default_ssh_auth_sock {
...
impl AgentProxy {
    ...
    fn update_now(&self) {
        ...
        clients.sort_by(|a, b| {
            ...
            // in onlyterm-mux-server-impl/src/sessionhandler.rs
