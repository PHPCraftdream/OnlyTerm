...
//! `crates/session/mux/src/localpane.rs`'s `tests` module (see the list in that
...
//! (`crates/apps/onlyterm-gui/src/termwindow/mod.rs`) performs against every
...
/// (`crates/apps/onlyterm-gui/src/termwindow/mod.rs`) reads off a pane on the
