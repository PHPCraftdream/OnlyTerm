...
impl Mux {
    pub fn new(default_domain: Option<Arc<dyn Domain>>) -> Self {
        ...
        let agent = if onlyterm_config::configuration().mux_enable_ssh_agent {
            Some(AgentProxy::new())
        } else {
            None
        };
    ...
    pub(super) fn get_default_workspace(&self) -> String {
    ...
    pub(in crate::mux) fn recompute_pane_count(&self) {
