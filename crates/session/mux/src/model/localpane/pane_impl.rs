...
impl Pane for LocalPane {
    ...
    /// `crates/terminal/term/src/terminalstate/keyboard.rs` and
    ...
    /// `crates/session/mux/src/domain.rs`) is itself non-blocking these days --
    ...
    /// (`crates/platform/pty/src/win/conpty.rs`, `take_writer()`) hands the pty's
        ...
        match &mut *proc {
            ProcessState::Running {
                ...
                if !*killed {
                    ...
                    // (see `WriterWrapper` in `crates/session/mux/src/domain.rs`)
