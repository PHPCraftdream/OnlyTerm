...
use onlyterm_procinfo::ProcessActivityStamp;
use std::sync::Arc;
...
#[derive(Clone)]
pub(in crate::model::localpane) struct ActivityState {
    epoch: Arc<()>,
...
impl ActivityState {
    pub(in crate::model::localpane) fn new(now: Instant) -> Self {
        Self {
            epoch: Arc::new(()),
            stamps: None,
            quiet_since: now,
...
mod tests {
    ...
    fn root() -> LocalProcessInfo {
        LocalProcessInfo {
            ...
            argv: vec![],
            cwd: "work".into(),
            status: onlyterm_procinfo::LocalProcessStatus::Run,
            start_time: 7,
            console: 1,
