...
fn get_title_does_not_block_on_a_locked_terminal() {
    ...
    // `onlyterm_promise::spawn::spawn_into_main_thread` -- that needs a
    // process-global scheduler installed (see
    // `onlyterm_promise::spawn::set_schedulers`), which is orthogonal to what
...
fn render_budget_exceeded_expires_once_painting_stops() {
    ...
    // (what `crates/apps/onlyterm-gui/src/termwindow/render/pane.rs` does
...
/// (`crates/platform/pty/src/win/conpty.rs`) hands the pty's stdin
