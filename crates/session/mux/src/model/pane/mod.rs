...
use onlyterm_config::keyassignment::{KeyAssignment, ScrollbackEraseMode};
...
pub trait Pane: Downcast + Send + Sync {
    ...
    /// `try_lock_terminal_for` in `crates/session/mux/src/localpane.rs`, task
    ...
    fn get_foreground_process_info(
        &self,
        _policy: CachePolicy,
    ) -> Option<onlyterm_procinfo::LocalProcessInfo> {
        None
    }
