...
use onlyterm_config::{configuration, ExecDomain, SerialDomain, ValueOrFunc};
...
impl Domain for LocalDomain {
    async fn spawn_pane(
        ...
        let starting_cwd = cmd
            ...
            .unwrap_or_else(|| onlyterm_config::HOME_DIR.clone());
            ...
            .openpty(crate::terminal_size_to_pty_size(size)?)
        {
            ...
            Err(err) => {
                ...
                let mut terminal = onlyterm_term::Terminal::new_with_nonblocking_writer(
                    size,
                    std::sync::Arc::new(onlyterm_config::TermConfig::new()),
                    "OnlyTerm",
