...
mod tests {
    ...
    #[test]
    fn empty_single_leaf_and_split_boundaries() {
        ...
        let single = make_single();
        assert_eq!(single.num_leaves(), 1);
        assert!(make_single().cursor().go_to_nth_leaf(1).is_err());

        let split_left = single
            .cursor()
            .split_leaf_and_insert_left(1)
            .unwrap()
            .tree();
        assert_eq!(
            split_left,
    ...
    #[test]
    fn parent_path_branches_and_leaf_order_are_stable() {
        let make_tree = || Tree::<i32, i32>::Node {
            left: Box::new(Tree::Leaf(1)),
        ...
        let right_leaf = make_tree().cursor().go_to_nth_leaf(2).unwrap();
        assert_eq!(
            right_leaf
                .path_to_root()
                .map(|(branch, _)| branch)
                .collect::<Vec<_>>(),
            vec![PathBranch::IsRight, PathBranch::IsRight]
        );

        let left_nested_leaf = make_tree().cursor().go_to_nth_leaf(1).unwrap();
        assert_eq!(
            left_nested_leaf
                .path_to_root()
                .map(|(branch, _)| branch)
                .collect::<Vec<_>>(),
            vec![PathBranch::IsLeft, PathBranch::IsRight]
        );

        let mut cursor = make_tree().cursor();
