...
//! The idea is to use these when onlyterm needs to request
...
pub struct TermWizTerminalPane {
    pane_id: PaneId,
    domain_id: DomainId,
    terminal: Mutex<onlyterm_term::Terminal>,
...
impl TermWizTerminalPane {
    fn new(
        ...
        let terminal = Mutex::new(onlyterm_term::Terminal::new(
            size,
            term_config.unwrap_or_else(|| Arc::new(onlyterm_config::TermConfig::new())),
            "OnlyTerm",
...
pub fn allocate(
    ...
    let renderer = onlyterm_termwiz_funcs::new_onlyterm_terminfo_renderer();
...
>(
    ...
    let (pane_id, window_id) = onlyterm_promise::spawn::spawn_into_main_thread(async move {
        register_tab(input_tx, render_rx, size, window_id, term_config).await
    })
    .await?;

    let result = onlyterm_promise::spawn::spawn_into_new_thread(move || f(tw_term)).await;
