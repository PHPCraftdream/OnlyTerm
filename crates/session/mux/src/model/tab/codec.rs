use super::{SplitDirectionAndSize, TabId};
use crate::pane::PaneId;
use crate::renderable::StableCursorPosition;
use crate::WindowId;
use onlyterm_term::{StableRowIndex, TerminalSize};
use serde::{Deserialize, Serialize};
use url::Url;

/// This type is used directly by the codec, take care to bump
/// the codec version if you change this
#[derive(Deserialize, Serialize, PartialEq, Debug)]
pub enum PaneNode {
    Empty,
    Split {
        left: Box<PaneNode>,
        right: Box<PaneNode>,
        node: SplitDirectionAndSize,
    },
    Leaf(PaneEntry),
}

impl PaneNode {
    pub fn into_tree(self) -> super::BinaryTree<PaneEntry, SplitDirectionAndSize> {
        match self {
            PaneNode::Empty => super::BinaryTree::Empty,
            PaneNode::Split { left, right, node } => super::BinaryTree::Node {
                left: Box::new((*left).into_tree()),
                right: Box::new((*right).into_tree()),
                data: Some(node),
            },
            PaneNode::Leaf(e) => super::BinaryTree::Leaf(e),
        }
    }

    pub fn root_size(&self) -> Option<TerminalSize> {
        match self {
            PaneNode::Empty => None,
            PaneNode::Split { node, .. } => Some(node.size()),
            PaneNode::Leaf(entry) => Some(entry.size),
        }
    }

    pub fn window_and_tab_ids(&self) -> Option<(WindowId, TabId)> {
        match self {
            PaneNode::Empty => None,
            PaneNode::Split { left, right, .. } => match left.window_and_tab_ids() {
                Some(res) => Some(res),
                None => right.window_and_tab_ids(),
            },
            PaneNode::Leaf(entry) => Some((entry.window_id, entry.tab_id)),
        }
    }
}

/// This type is used directly by the codec, take care to bump
/// the codec version if you change this
#[derive(Deserialize, Serialize, PartialEq, Debug, Clone)]
pub struct PaneEntry {
    pub window_id: WindowId,
    pub tab_id: TabId,
    pub pane_id: PaneId,
    pub title: String,
    pub size: TerminalSize,
    pub working_dir: Option<SerdeUrl>,
    pub is_active_pane: bool,
    pub is_zoomed_pane: bool,
    pub workspace: String,
    pub cursor_pos: StableCursorPosition,
    pub physical_top: StableRowIndex,
    pub top_row: usize,
    pub left_col: usize,
    pub tty_name: Option<String>,
}

#[derive(Deserialize, Clone, Serialize, PartialEq, Debug)]
#[serde(try_from = "String", into = "String")]
pub struct SerdeUrl {
    pub url: Url,
}

impl std::convert::TryFrom<String> for SerdeUrl {
    type Error = url::ParseError;
    fn try_from(s: String) -> Result<SerdeUrl, url::ParseError> {
        let url = Url::parse(&s)?;
        Ok(SerdeUrl { url })
    }
}

impl From<Url> for SerdeUrl {
    fn from(url: Url) -> SerdeUrl {
        SerdeUrl { url }
    }
}

impl From<SerdeUrl> for Url {
    fn from(s: SerdeUrl) -> Url {
        s.url
    }
}

impl From<SerdeUrl> for String {
    fn from(s: SerdeUrl) -> String {
        s.url.as_str().into()
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn pane_node_into_tree_preserves_split_shape_and_order() {
        let node = SplitDirectionAndSize {
            direction: super::super::SplitDirection::Horizontal,
            first: TerminalSize::default(),
            second: TerminalSize::default(),
        };
        let pane_node = PaneNode::Split {
            left: Box::new(PaneNode::Empty),
            right: Box::new(PaneNode::Empty),
            node,
        };

        assert_eq!(
            pane_node.into_tree(),
            super::super::BinaryTree::Node {
                left: Box::new(super::super::BinaryTree::Empty),
                right: Box::new(super::super::BinaryTree::Empty),
                data: Some(node),
            }
        );
    }
}
