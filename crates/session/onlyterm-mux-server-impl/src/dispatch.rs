...
fn handle_notification(
    ...
    match notif {
        ...
        MuxNotification::PaneRemoved(pane_id) => {
            send_pdu(
                write_tx,
                Pdu::PaneRemoved(onlyterm_codec::PaneRemoved { pane_id }),
            );
        }
        MuxNotification::AssignClipboard {
            ...
            send_pdu(
                write_tx,
                Pdu::SetClipboard(onlyterm_codec::SetClipboard {
                    pane_id,
                    clipboard,
                    selection,
                }),
        ...
        MuxNotification::TabAddedToWindow { tab_id, window_id } => {
            send_pdu(
                write_tx,
                Pdu::TabAddedToWindow(onlyterm_codec::TabAddedToWindow { tab_id, window_id }),
            );
        }
        MuxNotification::WindowWorkspaceChanged(window_id) => {
            ...
            if let Some(workspace) = workspace {
                send_pdu(
                    write_tx,
                    Pdu::WindowWorkspaceChanged(onlyterm_codec::WindowWorkspaceChanged {
                        window_id,
                        workspace,
                    }),
                );
        ...
        MuxNotification::PaneFocused(pane_id) => {
            send_pdu(
                write_tx,
                Pdu::PaneFocused(onlyterm_codec::PaneFocused { pane_id }),
            );
        }
        MuxNotification::TabReflowed(tab_id) => {
            send_pdu(
                write_tx,
                Pdu::TabReflowed(onlyterm_codec::TabReflowed { tab_id }),
            );
        }
        MuxNotification::TabTitleChanged { tab_id, title } => {
            send_pdu(
                write_tx,
                Pdu::TabTitleChanged(onlyterm_codec::TabTitleChanged { tab_id, title }),
            );
        }
        MuxNotification::WindowTitleChanged { window_id, title } => {
            send_pdu(
                write_tx,
                Pdu::WindowTitleChanged(onlyterm_codec::WindowTitleChanged { window_id, title }),
            );
        }
        MuxNotification::WorkspaceRenamed {
            ...
            send_pdu(
                write_tx,
                Pdu::RenameWorkspace(onlyterm_codec::RenameWorkspace {
                    old_workspace,
                    new_workspace,
                }),
            );
