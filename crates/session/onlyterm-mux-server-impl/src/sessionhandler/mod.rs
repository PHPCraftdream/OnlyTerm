...
use onlyterm_promise::spawn::spawn_into_main_thread;
...
impl SessionHandler {
    ...
    pub fn process_one(&mut self, decoded: DecodedPdu) {
            ...
            .authorize(&decoded.pdu, &Mux::get(), self.split_tracker.active())
        {
            ...
            sender
                .send(DecodedPdu {
                    pdu: onlyterm_codec::Pdu::ErrorResponse(onlyterm_codec::ErrorResponse {
                        reason: format!(
            ...
            Pdu::SetFocusedPane(SetFocusedPane { pane_id }) => {
                ...
                spawn_into_main_thread(async move {
                        ...
                        move || {
                            ...
                            mux.notify(onlyterm_mux::MuxNotification::PaneFocused(pane_id));
                ...
                match std::env::current_exe().context("resolving current_exe") {
                    ...
                    Ok(executable_path) => {
                        send_response(Ok(Pdu::GetCodecVersionResponse(GetCodecVersionResponse {
                            ...
                            version_string: onlyterm_config::onlyterm_version().to_owned(),
                            executable_path,
