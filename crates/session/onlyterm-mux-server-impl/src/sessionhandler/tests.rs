...
fn test_pdu_policy_unrestricted_allows_all() {
    ...
    assert!(policy.is_allowed(&onlyterm_codec::Pdu::Ping(Ping {})));

    // The most dangerous PDU should be allowed in unrestricted mode
    assert!(
        policy.is_allowed(&onlyterm_codec::Pdu::SpawnV2(onlyterm_codec::SpawnV2 {
...
fn test_pdu_policy_elevated_allowlist_allows_only_allowed_pdus() {
    ...
    assert!(policy.is_allowed(&onlyterm_codec::Pdu::GetCodecVersion(GetCodecVersion {})));
    assert!(
        policy.is_allowed(&onlyterm_codec::Pdu::SetClientId(SetClientId {
            client_id: onlyterm_mux::client::ClientId::default(),
            is_proxy: false,
        }))
    );
    assert!(policy.is_allowed(&onlyterm_codec::Pdu::Ping(Ping {})));
    assert!(policy.is_allowed(&onlyterm_codec::Pdu::ListPanes(ListPanes {})));
    assert!(
        policy.is_allowed(&onlyterm_codec::Pdu::GetPaneRenderChanges(
            GetPaneRenderChanges { pane_id: 0 }
        ))
    );
    assert!(policy.is_allowed(&onlyterm_codec::Pdu::GetLines(GetLines {
        pane_id: 0,
        lines: vec![],
    })));
    assert!(
        policy.is_allowed(&onlyterm_codec::Pdu::WriteToPane(WriteToPane {
            pane_id: 0,
            data: vec![],
        }))
    );
    assert!(
        policy.is_allowed(&onlyterm_codec::Pdu::SendPaste(SendPaste {
            pane_id: 0,
            data: "".to_string(),
        }))
    );
    assert!(policy.is_allowed(&onlyterm_codec::Pdu::Resize(Resize {
        pane_id: 0,
        containing_tab_id: 0,
        size: Default::default(),
    })));
    assert!(policy.is_allowed(&onlyterm_codec::Pdu::KillPane(KillPane { pane_id: 0 })));
    ...
    assert!(
        policy.is_allowed(&onlyterm_codec::Pdu::SendKeyDown(SendKeyDown {
            pane_id: 0,
            event: termwiz::input::KeyEvent {
                key: termwiz::input::KeyCode::Char('a'),
                modifiers: termwiz::input::Modifiers::NONE,
            },
            input_serial: onlyterm_codec::InputSerial::now(),
        }))
    );
    assert!(
        policy.is_allowed(&onlyterm_codec::Pdu::SendMouseEvent(SendMouseEvent {
            pane_id: 0,
            event: onlyterm_term::input::MouseEvent {
                kind: onlyterm_term::input::MouseEventKind::Press,
                x: 0,
                y: 0,
                x_pixel_offset: 0,
                y_pixel_offset: 0,
                button: onlyterm_term::input::MouseButton::Left,
                modifiers: onlyterm_term::input::KeyModifiers::NONE,
            },
        }))
    );
}
...
fn test_pdu_policy_elevated_allowlist_rejects_dangerous_pdus() {
    ...
    assert!(!policy.is_allowed(&onlyterm_codec::Pdu::MovePaneToNewTab(
        onlyterm_codec::MovePaneToNewTab {
            pane_id: 0,
            window_id: None,
    ...
    assert!(
        !policy.is_allowed(&onlyterm_codec::Pdu::SetPaneZoomed(SetPaneZoomed {
            pane_id: 0,
            containing_tab_id: 0,
            zoomed: false,
        }))
    ...
    assert!(
        !policy.is_allowed(&onlyterm_codec::Pdu::RotatePanes(RotatePanes {
            pane_id: 0,
            direction: onlyterm_config::keyassignment::RotationDirection::Clockwise,
        }))
    );
    assert!(
        !policy.is_allowed(&onlyterm_codec::Pdu::SwapActivePaneWithIndex(
    ...
    assert!(
        !policy.is_allowed(&onlyterm_codec::Pdu::ActivatePaneDirection(
            ActivatePaneDirection {
                pane_id: 0,
                direction: onlyterm_config::keyassignment::PaneDirection::Up,
            }
        ))
    );
    assert!(
        !policy.is_allowed(&onlyterm_codec::Pdu::AdjustPaneSize(AdjustPaneSize {
            pane_id: 0,
            direction: onlyterm_config::keyassignment::PaneDirection::Up,
            amount: 0,
        }))
    ...
    assert!(!policy.is_allowed(&onlyterm_codec::Pdu::SetWindowWorkspace(
        SetWindowWorkspace {
            window_id: 0,
            workspace: "".to_string(),
        }
    ...
    assert!(
        !policy.is_allowed(&onlyterm_codec::Pdu::RenameWorkspace(RenameWorkspace {
    ...
    assert!(!policy.is_allowed(&onlyterm_codec::Pdu::GetClientList(GetClientList)));

    // Scrollback manipulation
    assert!(
        !policy.is_allowed(&onlyterm_codec::Pdu::EraseScrollbackRequest(
            EraseScrollbackRequest {
                pane_id: 0,
                erase_mode: onlyterm_config::keyassignment::ScrollbackEraseMode::ScrollbackOnly,
            }
        ))
    );
    assert!(
        !policy.is_allowed(&onlyterm_codec::Pdu::SearchScrollbackRequest(
            SearchScrollbackRequest {
                pane_id: 0,
                pattern: onlyterm_mux::pane::Pattern::CaseSensitiveString("".to_string()),
                range: 0..0,
                limit: None,
            }
        ))
    ...
    // test_pdu_policy_elevated_allowlist_allows_only_allowed_pdus.
    assert!(!policy.is_allowed(&onlyterm_codec::Pdu::GetTlsCreds(GetTlsCreds {})));
...
fn test_pdu_policy_elevated_allowlist_allows_presentation_state() {
    ...
    assert!(
        policy.is_allowed(&onlyterm_codec::Pdu::SetPalette(SetPalette {
            pane_id: 0,
            palette: Default::default(),
        }))
    );
    assert!(
        policy.is_allowed(&onlyterm_codec::Pdu::SetFocusedPane(SetFocusedPane {
            pane_id: 0
        }))
    );
    assert!(policy.is_allowed(&onlyterm_codec::Pdu::WindowTitleChanged(
        WindowTitleChanged {
            window_id: 0,
            title: "".to_string(),
        }
    ...
    assert!(
        policy.is_allowed(&onlyterm_codec::Pdu::TabTitleChanged(TabTitleChanged {
            tab_id: 0,
            title: "".to_string(),
        }))
    );
...
impl onlyterm_mux::pane::Pane for FixedPane {
    fn pane_id(&self) -> onlyterm_mux::pane::PaneId {
        1
    }
    ...
    fn get_cursor_position(&self) -> onlyterm_mux::renderable::StableCursorPosition {
        onlyterm_mux::renderable::StableCursorPosition::default()
    }
    ...
    ) -> onlyterm_rangeset::RangeSet<StableRowIndex> {
        onlyterm_rangeset::RangeSet::new()
    }
    ...
    fn with_lines_mut(
        &self,
        lines: std::ops::Range<StableRowIndex>,
        with_lines: &mut dyn onlyterm_mux::pane::WithPaneLines,
    ) {
        onlyterm_mux::pane::impl_with_lines_via_get_lines(self, lines, with_lines);
    }

    fn for_each_logical_line_in_stable_range_mut(
        &self,
        lines: std::ops::Range<StableRowIndex>,
        for_line: &mut dyn onlyterm_mux::pane::ForEachPaneLogicalLine,
    ) {
        onlyterm_mux::pane::impl_for_each_logical_line_via_get_logical_lines(self, lines, for_line);
    }

    fn get_logical_lines(
        &self,
        lines: std::ops::Range<StableRowIndex>,
    ) -> Vec<onlyterm_mux::pane::LogicalLine> {
        onlyterm_mux::pane::impl_get_logical_lines_via_get_lines(self, lines)
    }

    fn get_dimensions(&self) -> onlyterm_mux::renderable::RenderableDimensions {
        onlyterm_mux::renderable::RenderableDimensions {
            cols: 80,
            viewport_rows: 24,
            scrollback_rows: 24,
            physical_top: 0,
    ...
    fn resize(&self, _size: onlyterm_term::TerminalSize) -> anyhow::Result<()> {
        Ok(())
    }

    fn key_down(
        &self,
        _key: onlyterm_term::KeyCode,
        _mods: onlyterm_term::KeyModifiers,
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    fn mouse_event(&self, _event: onlyterm_term::MouseEvent) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    fn palette(&self) -> onlyterm_term::color::ColorPalette {
        onlyterm_term::color::ColorPalette::default()
    }

    fn domain_id(&self) -> onlyterm_mux::domain::DomainId {
        0
    }
    ...
    fn get_current_working_dir(
        &self,
        _policy: onlyterm_mux::pane::CachePolicy,
    ) -> Option<url::Url> {
        None
    }
}
...
fn keyboard_encoding_change_alone_produces_a_push() {
    ...
    let pane: std::sync::Arc<dyn onlyterm_mux::pane::Pane> = pane.clone();
