...
fn main() {
    ...
    println!("//! cd ../codegen ; cargo run > ../src/emoji_variation.rs");
    println!();
    emit_variation_map();
    emit_classify_table();
...
fn generate_nerdfonts_data() {
    ...
    let mut symbols: Vec<(String, char)> = parsed.glyphs.into_iter().map(|(name, g)| (name.replace("-", "_"), g.glyph_text)).collect();
...
fn emit_variation_map() {
    ...
    println!();
    println!(
        "pub const VARIATION_MAP: phf::Map<&'static str, (Presentation, Presentation)> = \n{};\n",
        map.build(),
    );
}
