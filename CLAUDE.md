...
- При уровне Info GUI пишет Info асинхронно: аварийное завершение может потерять
  хвост очереди; гарантированной границы потерь по времени нет. Warn/Error пишутся
  синхронно после слива очереди. Для диагностики воспроизводимого падения запускай
  с `ONLYTERM_LOG=debug` или `trace` — это включает синхронный вывод всех записей.
  Отсутствие записи в логе не доказывает, что событие не произошло; flush не является
  гарантией сохранности при отключении питания или ошибке диска.
