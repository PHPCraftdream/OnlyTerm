...
## Предпосылки и порядок с соседними планами

Сначала принять и реализовать выбранные этапы [консолидации пакетов](2026-09-24-crate-consolidation.md), затем [переименования собственных пакетов](2026-09-24-crate-naming.md), затем выполнять **только перенос каталогов** по этому плану. Если какой-то этап объединения/переименования отклонён, его не считать выполненным ради переноса. Разделение изменений важно: на границе переноса состав и имена пакетов уже должны быть определены, а сам перенос не меняет ни то ни другое.

Непосредственно перед физическим переносом заново получить `cargo metadata` и составить по нему актуальную карту «существующий пакет → текущая папка → целевая группа». Удалить из нижеследующей таблицы пакеты, действительно поглощённые другим crate; для утверждённых новых имён подставить имена из актуальных манифестов. Если при переименовании сменились и имена каталогов, исходные пути также брать из нового дерева. Числа 54 и 57 и все строки таблицы ниже — проверенный снимок на 2026-09-24, а не неизменный критерий приёмки. Сверить уникальность всех источников/назначений и отдельно учесть исключённые генераторы и vendored patch.

## Целевое дерево

Ниже перечислены **все 54 члена workspace в текущем снимке**. Путь справа задан относительно корня репозитория; вложенные `derive` и `generate` перемещаются вместе с родителем. Группы имеют лишь один дополнительный уровень под `crates/`. При реализации после соседних планов применять правило пересоставления карты выше.
...
Точная проверка инварианта: сохранить до и после JSON-вывод `cargo metadata --no-deps --format-version 1`, причём `before` получить **после** завершения принятых соседних планов и **до** первого `git mv`. Скрипт должен сравнивать с этим baseline число и множество имён пакетов, `version`, имена/типы targets, нормализованные зависимости и отсортированное множество имён `workspace_members`. Исключённый `onlyterm-char-props/codegen` (если остаётся) и patch проверять отдельно по их `Cargo.toml` и разрешению `cargo metadata`, поскольку они не входят в workspace. Это ловит случайно потерянный или вновь добавленный пакет лучше одной проверки `cargo check`.
...
before = projection(sys.argv[1])
after = projection(sys.argv[2])
assert before == after
print(f"{len(before[0])} workspace packages and manifest semantics unchanged")
```
