...
The *cli* subcommand interacts with a running onlyterm GUI or multiplexer
...
onlyterm uses the following logic to decide which one it should connect to.
...
* If the `$ONLYTERM_UNIX_SOCKET` environment variable is set, use that location
...
* If the `$ONLYTERM_PANE` environment variable is set, it will be used
...
See also: [onlyterm cli list](list.md)
