...
*Run `onlyterm cli spawn --help` to see more help*
...
to `onlyterm cli spawn` so that any parameters you may wish to pass to the
program are not confused with parameters to `onlyterm cli spawn`.  This example
launches `top` in a new tab:

```
$ onlyterm cli spawn -- top
2
```
...
```console
{% include "../../examples/cmd-synopsis-onlyterm-cli-spawn--help.txt" %}
```
