# `onlyterm cli split-pane`

*Run `onlyterm cli split-pane --help` to see more help*
...
to `onlyterm cli split-pane` so that any parameters you may wish to pass to the
program are not confused with parameters to `onlyterm cli split-pane`.  This example
launches bash as a login shell in a new pane at the bottom:

```
$ onlyterm cli split-pane -- bash -l
3
```
...
```
$ onlyterm cli split-pane --left --percent 30
4
```
...
```console
{% include "../../examples/cmd-synopsis-onlyterm-cli-split-pane--help.txt" %}
```
