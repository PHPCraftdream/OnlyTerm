...
when running the `onlyterm start` subcommand.

It is triggered before any default program is started.

If no explicit program was passed to `onlyterm start`, and if the
...
This event does not fire for `onlyterm connect` invocations.

{{since('20220807-113146-c2fee766')}}

The event receives an optional [SpawnCommand](../SpawnCommand.md) argument that
corresponds to any arguments that may have been passed via `onlyterm start`.
In earlier releases if you implemented this event, you would essentially
prevent `onlyterm start -- something` from spawning `something`.
...
onlyterm.on('gui-startup', function(cmd)
...
onlyterm.on('gui-startup', function(cmd)
  -- allow `onlyterm start -- something` to affect what we spawn
  ...
  local project_dir = onlyterm.home_dir .. '/onlyterm'
