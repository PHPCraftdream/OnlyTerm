...
* `is_unresponsive` - true if a recent attempt to read this pane's title/progress/user-vars/current-working-directory gave up and served stale data instead, per [pane:is_unresponsive()](pane/is_unresponsive.md). {{since('nightly', inline=True)}}
