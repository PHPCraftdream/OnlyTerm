...
!!! warning "Pending rhai conversion"
    ...
    The examples on this page still show the old Lua `wezterm.exec_domain(NAME,
    FIXUP, LABEL)` factory-function API, where *FIXUP*/*LABEL* were inline Lua
    closures. In the rhai engine, `ExecDomain` is deserialized directly from an
    object map (`#{ name: ..., fixup_command: ..., label: ... }`), which is a
    real shape change, not just a syntax translation, and the exact rhai-facing
    field semantics need to be confirmed against `crates/config/src/exec_domain.rs`
    before rewriting these examples. See the [migration
    guide](../../migration-lua-to-rhai.md) for the general Lua-to-rhai mapping;
    this specific page is still pending a verified rewrite.
