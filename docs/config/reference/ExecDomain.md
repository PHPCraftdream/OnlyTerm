...
onlyterm to explicitly spawn a command that runs `docker exec`, you would also
...
You must use the `onlyterm.exec_domain` function to define a domain. It accepts
the following parameters:

```
onlyterm.exec_domain(NAME, FIXUP [, LABEL])
```
...
```lua
onlyterm.exec_domain('myname', function(cmd)
  return cmd
end)
```
...
what onlyterm will execute in order to satisfy the user's request to
spawn a new program.
...
-- onlyterm.exec_domain()
onlyterm.exec_domains(domain_name, fixup_func, function(domain_name)
  return domain_name
end)
```
...
[onlyterm.format()](onlyterm/format.md) to manage that.
...
config.exec_domains = {
  ...
  -- killed.
  onlyterm.exec_domain('scoped', function(cmd)
    -- The "cmd" parameter is a SpawnCommand object.
    -- You can log it to see what's inside:
    onlyterm.log_info(cmd)

    -- Synthesize a human understandable scope name that is
    -- (reasonably) unique. ONLYTERM_PANE is the pane id that
    -- will be used for the newly spawned pane.
    -- ONLYTERM_UNIX_SOCKET is associated with the onlyterm
    -- process id.
    local env = cmd.set_environment_variables
    local ident = 'onlyterm-pane-'
      .. env.ONLYTERM_PANE
      .. '-on-'
      .. basename(env.ONLYTERM_UNIX_SOCKET)
    ...
    local wrapped = {
      ...
      '--user',
      '--scope',
      '--description=Shell started by onlyterm',
      '--same-dir',
      '--collect',
      '--unit=' .. ident,
    }
...
-- spawned by onlyterm will have its own scope
...
function docker_list()
  local docker_list = {}
  local success, stdout, stderr = onlyterm.run_child_process {
    'docker',
    'container',
    'ls',
    '--format',
  ...
  for _, line in ipairs(onlyterm.split_by_newlines(stdout)) do
