...
also recognized by onlyterm; this example sets the `foo` user variable
to the value `bar`:
...
__onlyterm_set_user_var "foo" "bar"
```

you're then able to access this in your onlyterm config:

```lua
onlyterm.log_info('foo var is ' .. pane:get_user_vars().foo)
```
