...
[onlyterm.home_dir](../onlyterm/home_dir.md) to explicitly refer to your home
directory.

If `onlyterm start --cwd /some/path` is used to specify the current working
...
On macOS and Linux, `onlyterm` can attempt to resolve the process group leader
...
On Windows, there isn't a process group leader concept, but `onlyterm` will
