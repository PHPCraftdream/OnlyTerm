...
A layer is an object with the following fields:
...
!!! note "No variables in ktav"

    The original version of this example declared a Lua local variable,
    `dimmer`, once and reused it across all five layers below. ktav has no
    variables or references, so the same `hsb: { brightness: 0.1 }` value is
    written out literally at each layer instead.

```
## The art is a bit too bright and colorful to be useful as a backdrop
## for text, so we're going to dim it down to 10% of its normal brightness
## (repeated literally below, since ktav has no variables to share it via).
