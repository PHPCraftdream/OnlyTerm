...
update (DEC private mode 2026) open before onlyterm stops waiting for the
closing sequence and applies the buffered output anyway.

While a synchronized update is open, onlyterm holds back output so that the
...
applications more time to complete an atomic frame; lowering it makes onlyterm
