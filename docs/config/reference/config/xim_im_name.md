...
Explicitly set the name of the IME server to which onlyterm will connect
via the XIM protocol when using X11 and [use_ime](use_ime.md) is `true`.

By default, this option is not set which means that onlyterm will consider
...
will cause onlyterm to connect to fcitx regardless of the value of `XMODIFIERS`.
