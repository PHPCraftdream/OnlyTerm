...
live resize operation.  `onlyterm` will coalesce the stream of multiple events
...
onlyterm.on('window-resized', function(window, pane)
  recompute_padding(window)
end)

onlyterm.on('window-config-reloaded', function(window)
  recompute_padding(window)
end)
```
