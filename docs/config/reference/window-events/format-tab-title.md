...
[onlyterm.run_child_process](../onlyterm/run_child_process.md)) are not possible
...
-- or `onlyterm cli set-tab-title`, but falls back to the
...
* a table holding `FormatItem`s as used in the [onlyterm.format](../onlyterm/format.md) function.  This allows formatting style and color information for individual elements within the tab.
...
to [tab_max_width](../config/tab_max_width.md).  OnlyTerm will then compute
...
`onlyterm.on("format-tab-title", ...)` calls.
...
local SOLID_LEFT_ARROW = onlyterm.nerdfonts.pl_right_hard_divider

-- The filled in variant of the > symbol
local SOLID_RIGHT_ARROW = onlyterm.nerdfonts.pl_left_hard_divider
...
onlyterm.on(
  ...
  function(tab, tabs, panes, config, hover, max_width)
    ...
    title = onlyterm.truncate_right(title, max_width - 2)
