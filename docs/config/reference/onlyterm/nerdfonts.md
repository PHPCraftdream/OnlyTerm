...
status lines and titles in your onlyterm configuration.

OnlyTerm includes `Nerd Font Symbols Font` as a default font fallback
which means that these special symbols are available even without 
requiring you to use a patched font.

This example shows how to lookup `onlyterm.nerdfonts.fa_clock_o`, the Font
...
onlyterm.on('update-right-status', function(window, pane)
  ...
  window:set_right_status(onlyterm.format {
    { Text = onlyterm.nerdfonts.fa_clock_o .. ' ' .. date },
  })
end)
```
