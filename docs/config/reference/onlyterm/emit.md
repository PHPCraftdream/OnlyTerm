...
`onlyterm.emit` resolves the registered callback(s) for the specified
...
being called for this particular call to `onlyterm.emit`, and `onlyterm.emit`
will return `false` to indicate that no additional/default processing
should take place.

If none of the callbacks returned `false` then `onlyterm.emit` will
...
onlyterm, or what their required arguments might be.

See [onlyterm.on](on.md) for more information about event handling.
