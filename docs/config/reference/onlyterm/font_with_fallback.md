---
title: onlyterm.font_with_fallback
tags:
 - font
---

# `onlyterm.font_with_fallback(families [, attributes])`
...
This function constructs a rhai map that configures a font with fallback processing.
Glyphs are looked up in the first font in the list but if missing the next font is
checked and so on.

The first parameter is an array listing the fonts in their preferred order:

```rhai
return #{
  font: font_with_fallback([ "JetBrains Mono", "Noto Color Emoji" ]),
}
```

OnlyTerm implicitly adds its default fallback to the list that you specify.

The *attributes* parameter behaves the same as that of [onlyterm.font](font.md)
...
form where the family and attributes are specified as part of the same rhai map:
...
= `true` will cause onlyterm to try to automatically scale using the
