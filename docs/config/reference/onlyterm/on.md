...
tags:
 ...
 - event
---
# `onlyterm.on(event_name, callback)`
...
`onlyterm.on` causes your specified `callback` to be called when `event_name`
is emitted.  Events can be emitted by onlyterm itself, or through code/configuration
that you specify.

`onlyterm.on` can register multiple callbacks for the same event; internally
...
See [onlyterm.action_callback](./action_callback.md) for a helper to define a custom action callback.
...
You may register handlers for arbitrary events for which onlyterm itself
has no special knowledge.  It is recommended that you avoid event names
that are likely to be used future versions of onlyterm in order to avoid
unexpected behavior if/when those names might be used in future.

The [onlyterm.emit](emit.md) function and the [EmitEvent](../keyassignment/EmitEvent.md) key assignment can be used to
emit events.
...
onlyterm.on('trigger-vim-with-scrollback', function(window, pane)
