...
# `onlyterm.font(family [, attributes])`
...
This function constructs a rhai map that corresponds to the internal `FontAttributes`
...
oblique) for non-bitmap fonts, onlyterm can only select and use fonts that you
...
```rhai
return #{
  font: font("JetBrains Mono", #{ weight: "Bold" }),
}
```

When resolving fonts from [font_dirs](../config/font_dirs.md), onlyterm follows CSS Fonts
...
are combined in the same rhai map.  This form is most useful when used together with
[font_with_fallback](font_with_fallback.md) when you want to specify precise
