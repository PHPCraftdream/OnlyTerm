...
The default encoding scheme used by onlyterm is xterm compatible, but there are
...
When onlyterm receives the sequence `CSI >4;Nm`, where `N` is `0`, `1` or `2`,
...
Note that [enable_csi_u_key_encoding](reference/config/enable_csi_u_key_encoding.md)
and [allow_win32_input_mode](reference/config/allow_win32_input_mode.md) both take
...
See also: [enable_csi_u_key_encoding](reference/config/enable_csi_u_key_encoding.md).

Note that [allow_win32_input_mode](reference/config/allow_win32_input_mode.md) takes
...
[enable_kitty_keyboard](reference/config/enable_kitty_keyboard.md) controls whether
onlyterm will honor the application requests to modify the keyboard encoding.

## Windows

On Windows, [allow_win32_input_mode](reference/config/allow_win32_input_mode.md)
defaults to `true` which causes onlyterm to listen for an escape sequence
...
[enable_csi_u_key_encoding](reference/config/enable_csi_u_key_encoding.md).
