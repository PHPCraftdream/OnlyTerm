...
64-bit Windows 10.0.17763 or later is required to run OnlyTerm; running on
earlier versions of Windows is not possible, as OnlyTerm requires [Pseudo
...
OnlyTerm is available in a setup.exe style installer; the installer is produced
with Inno Setup and will install onlyterm to your program files directory and
...
OnlyTerm is also available in a simple zip file that can be extracted and
run from anywhere, including a flash drive for a portable/relocatable
installation.

1. Download <a href="{{ windows_zip_stable }}">Release</a>
2. Extract the zipfile and double-click `onlyterm.exe` to run the UI
...
Once you have `winget`, you can install onlyterm like so:
...
OnlyTerm is available from the "Extras" bucket and once you have installed
scoop itself can be installed like so:

```console
$ scoop bucket add extras
$ scoop install onlyterm
```

### For `Chocolatey` users

If you prefer to use [Chocolatey](https://chocolatey.org) to manage software,
onlyterm is available from the Community Repository.  It can be installed like
so:

```console
$ choco install onlyterm -y
```
