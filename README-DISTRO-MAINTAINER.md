...
`ci/deploy.sh` is a script that is used to build packages in OnlyTerm's CI.
...
OnlyTerm's version number is derived from the date of the commit from which it
was released:
...
If you are not building OnlyTerm from its git repo, OnlyTerm will read a file named
...
If you must decorate OnlyTerm's version, then it is recommend that you append
your supplemental extra version information on the end of the OnlyTerm's
version.
...
* `onlyterm-mux-server` - multiplexer server. No gui required. Likely want this
  in a separate package from `onlyterm-gui` so it can run on a headless system.
* `onlyterm-gui` - the GUI portion of the terminal
* `onlyterm` - the CLI and frontend that knows how to launch the GUI. It is
...
* `assets/shell-integration`, `assets/shell-completion`: should be deployed along with the `onlyterm` executable
* `assets/wezterm.desktop`, `assets/wezterm.appdata.xml`, `assets/wezterm-nautilus.py`: should be deployed along with `onlyterm-gui`
...
By default, OnlyTerm will compile in a handful of fonts in order to provide a
...
Note that OnlyTerm requires at least the following fonts to be available, either
