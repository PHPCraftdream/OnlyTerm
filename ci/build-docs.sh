...
[[ -f /tmp/onlyterm.releases.json ]] || ghapi /repos/wezterm/wezterm/releases > /tmp/onlyterm.releases.json
[[ -f /tmp/onlyterm.nightly.json ]] || ghapi /repos/wezterm/wezterm/releases/tags/nightly > /tmp/onlyterm.nightly.json
...
cp "assets/icon/onlyterm-icon.svg" docs/favicon.svg
mkdir -p docs/fonts
...
docker_or_podman build -t onlyterm/mkdocs-material -f ci/Dockerfile.docs .

if [ "$SERVE" == "yes" ] ; then
  docker_or_podman run --rm -it -p8000:8000 -v ${PWD}:/docs onlyterm/mkdocs-material serve -a 0.0.0.0:8000
  #docker_or_podman run --rm -it --network=host -v ${PWD}:/docs onlyterm/mkdocs-material $@
else
  docker_or_podman run --rm -e CARDS=true -v ${PWD}:/docs onlyterm/mkdocs-material build
fi
